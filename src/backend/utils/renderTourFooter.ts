...
import type { FrameworkKey, RouteKey } from '../../shared/types';
...
const surfaceLabelFor = (key: RouteKey): { label: string; framework: FrameworkKey } => {
...
export const renderTourFooter = (current: RouteKey): string => {
	if (!TOUR_ORDER.includes(current)) return '';
	...
	const prevBtn = prev
		? renderBtn(prev, 'prev')
		: '<span class="ao-tour__placeholder" aria-hidden="true"></span>';
	const nextBtn = next
		? renderBtn(next, 'next')
		: '<span class="ao-tour__placeholder" aria-hidden="true"></span>';
