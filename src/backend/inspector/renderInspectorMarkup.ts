...
import type { FrameworkKey, InspectorMetadata, RouteKey } from '../../shared/types';
...
export const frameworkLabel = (key: FrameworkKey): string => FRAMEWORK_LABEL[key];

const row = (key: string, value: string) =>
	`<div class="ao-inspector__row"><div class="ao-inspector__key">${escape(key)}</div><div class="ao-inspector__val">${escape(value)}</div></div>`;

const panelMarkup = (m: InspectorMetadata) =>
...
export const inspectorPillLabel = (m: InspectorMetadata) =>
	`${frameworkLabel(m.framework)} · ${m.render.split(' ')[0] ?? m.render}`;

export const inspectorHeadScriptTag = () =>
	`<script>${INSPECTOR_INLINE_SCRIPT}</script>`;

export const renderInspectorMarkup = (key: RouteKey): string => {
